...
public class MySqlSessionFactoryBean  implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent> {
	  ...
	  public void refresh(String mappingName,InputStream mappInput) throws Exception{
	        try {
	          XMLMapperBuilder xmlMapperBuilder = new XMLMapperBuilder(mappInput,
	              configuration, mappingName, configuration.getSqlFragments());
	          xmlMapperBuilder.parse();
	        } catch (Exception e) {
	          throw new NestedIOException("Failed to parse mapping resource: '" + mappingName + "'", e);
	        } finally {
	          ErrorContext.instance().reset();
	        }
	        if (logger.isDebugEnabled()) {
	          logger.debug("Parsed mapper file: '" + mappingName + "'");
	        }
